...
function renderTasksAsText(tasks, projectsMetadata, settings) {
  function renderTaskFinishDate(task) {
    if (!task.dateCompleted)
      return "N/A";
    const m = (0, import_obsidian2.moment)(task.dateCompleted).local();
    const dt = m.format("YYYY-MM-DD HH:mm");
    const d = m.format("YYYY-MM-DD");
    const pf = settings.taskPostfix;
    if (/\{task_finish_datetime\}|\{current_datetime\}/.test(pf))
      return dt;
    if (/\{task_finish_date\}|\{current_date\}/.test(pf))
      return d;
    return dt;
  }
  function renderTaskPostfix(task) {
    return settings.taskPostfix.replace(/{task_finish_date}|{task_finish_datetime}|{current_date}|{current_datetime}/g, () => renderTaskFinishDate(task));
  }
  function renderTaskPrefix(_t, idx) {
  ...
  function renderTree(nodes, indent) {
    return [...nodes].reverse().flatMap((t, idx) => {
      const tabs = settings.renderProjectsHeaders ? "	".repeat(indent + 1) : "	".repeat(indent);
...
function getTimeframesForUsersToday() {
  ...
  const mStart = (0, import_obsidian3.moment)(startLocal);
  const mEnd = (0, import_obsidian3.moment)(endLocal);
  return {
...
function getTimeframesForLastNHours(hours) {
  const now = new Date();
  const startLocal = new Date(now.getTime() - hours * 60 * 60 * 1e3);
  const endLocal = now;
  const mStart = (0, import_obsidian3.moment)(startLocal);
  const mEnd = (0, import_obsidian3.moment)(endLocal);
  return {
...
function getTimeFromKeySegments(fileContent) {
  ...
  const mStart = (0, import_obsidian3.moment)(startTimeObj);
  const mEnd = (0, import_obsidian3.moment)(endTimeObj);
  if (mStart.format("YYYY-MM-DD") === "Invalid date" || mEnd.format("YYYY-MM-DD") === "Invalid date") {
    new import_obsidian3.Notice("Invalid date format. Please use 'YYYY-MM-DD HH:mm' format.", 1e4);
    return null;
  }
  return {
    timeStartFormattedDate: mStart.format("YYYY-MM-DD"),
...
This will limit the number of tasks fetched to 30.`, 15e3);
          }
